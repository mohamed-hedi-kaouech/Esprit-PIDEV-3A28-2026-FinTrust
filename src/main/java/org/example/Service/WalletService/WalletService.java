...
public class WalletService {
    ...
    public boolean ajouterWallet(Wallet wallet) {
        ...
        String query = "INSERT INTO wallet (nom_proprietaire, telephone, email, code_acces, est_actif, solde, devise, statut, date_creation) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
            ...
            pstmt.setString(7, wallet.getDevise().name());     // ← CORRECT
            pstmt.setString(8, wallet.getStatut().name());
            pstmt.setTimestamp(9, Timestamp.valueOf(wallet.getDateCreation()));
    ...
    public boolean modifierWallet(Wallet wallet) {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(query)) {
            ...
            pstmt.setString(5, wallet.getDevise().name());     // ← CORRECT
    ...
    public boolean mettreAJourWallet(Wallet wallet) {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(query)) {
            ...
            // ✅ Devise : utiliser name()
            pstmt.setString(6, wallet.getDevise().name());

            // ✅ STATUT : utiliser name() pour correspondre à l'ENUM
            pstmt.setString(7, wallet.getStatut().name());  // ← TRÈS IMPORTANT !
