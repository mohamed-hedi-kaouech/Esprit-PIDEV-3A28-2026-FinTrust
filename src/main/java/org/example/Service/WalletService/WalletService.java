...
public class WalletService implements InterfaceGlobal<Wallet> {
    ...
    public void Add(Wallet w) {
        ...
        try {
            ...
            // 🔥 ENUM → STRING
            ps.setString(3, w.getDevise().name());
            ps.setString(4, w.getStatut().name());
    ...
    public List<Wallet> ReadAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // 🔥 STRING → ENUM
                w.setDevise(WalletDevise.valueOf(rs.getString("devise")));
                w.setStatut(WalletStatut.valueOf(rs.getString("statut")));
